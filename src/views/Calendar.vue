...
async function listUpcomingEvents() {
  ...
  try {
    ...
    createAlert(
      "Try to reconnect on the profile page",
      "error",
      undefined,
      "Undefined tockenClient"
    );
    return;
  }
...
async function handleChevronClick(step) {
  sortingTime.value = stepSortingTime(
    sortingType.value,
    sortingTime.value,
    step
  );
...
<template>
  ...
  <section v-if="isConnected" id="section_calendar" class="px-4 w-full">
    ...
    <div
      v-if="sortingTime && sortingType"
      class="flex justify-center pt-4 gap-4 select-none"
    >
      <span
        class="material-symbols-outlined hover:cursor-pointer active:scale-95 flex items-center"
        @click="handleChevronClick(-1)"
        >arrow_back_ios</span
      >
      <input
        v-model="sortingTime"
    ...
    <div
      v-if="eventLoading"
      class="w-full fixed top-52 right-0 flex items-center justify-center"
    >
        ...
        <tr v-for="(week, weekIndex) in calendarData" :key="weekIndex">
          ...
          >
            <p
              v-if="day"
              :class="{
                'bg-green-200  size-5 flex justify-center items-center rounded-full text-zinc-900':
                  isTodayMonthly(sortingTime, day),
              }"
            >
              {{ day }}
            </p>
    ...
    <table
      v-else-if="sortingType == 'daily'"
      id="table_event_daily"
      v-if="!eventLoading"
    >
      <tr v-for="i in 24">
        <td class="w-14 text-right pr-2 text-xs">
          {{ i - 1 <= 11 ? i - 1 + " AM" : i - 1 + " PM" }}
        </td>
        <td class="second_row">
          <template v-for="event in eventsList">
            <EventCard
      ...
      <tbody>
        <tr>
          ...
          >
            <p
              :class="{
                'bg-green-200   flex justify-center items-center rounded-full text-zinc-900':
                  isTodayWeekly(sortingTime, day),
              }"
            >
              {{ truncatedDay(day) }}
    ...
    <div v-else class="buttonCalendarePageFixed">
      ...
      <button
        ...
        :disabled="getSortingTime(sortingType) === sortingTime"
